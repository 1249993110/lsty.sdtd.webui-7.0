<template>
    <div class="teleport-home-management">
        ...
        <el-table class="table-container" :data="tableData" border height="calc(100% - 84px)" highlight-current-row ref="tableRef">
            ...
            <el-table-column prop="platformId" label="平台Id" width="215px" sortable> </el-table-column>
        ...
        <AddOrEditHomePositions ref="addOrEditHomePositionsRef" @onSubmit="getData" />
    </div>
</template>
...
const getData = () => {
    api.getHomePositionPaged(params).then((data) => {
...
const tableRef = ref();
const addOrEditHomePositionsRef = ref();

const handleAdd = () => {
    addOrEditHomePositionsRef.value.show();
};

const handleEdit = (row) => {
    addOrEditHomePositionsRef.value.show(row);
};

const handleDelete = async (row) => {
    try {
        await myconfirm('确定删除选中内容吗?');
        await api.deleteHomePositionById(row.id);
        getData();
    } catch {}
};
</script>
