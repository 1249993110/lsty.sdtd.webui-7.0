<template>
    <el-card class="item-blocks">
        <div class="search-container">
            ...
            <el-input class="input" v-model="params.keyword" placeholder clearable @keyup.enter.native="search"></el-input>
...
const getData = async () => {
    ...
    try {
        if (itemsChecked.value && blocksChecked.value) {
            params.itemBlockKind = 0;
        } else if (itemsChecked.value && !blocksChecked.value) {
            params.itemBlockKind = 1;
        } else if (!itemsChecked.value && blocksChecked.value) {
            params.itemBlockKind = 2;
        } else {
            ElMessage.info('您必须至少选中一个种类');
            return;
        }

        params.showUserHidden = devItemsChecked.value;

        const data = (await getItemBlocks(params)).items;
